...
public sealed class TrendDetector : ITrendDetector
{
    ...
    private readonly ITrendConfiguration _config;
    private readonly ITrendStateProvider _stateProvider;
    ...
    public TrendDetector(
        ...
        ITrendConfiguration config,
        ITrendStateProvider stateProvider,
    ...
    {
        // Trend options accessed directly via _config
        var currentState = _stateProvider.CurrentTrendState;
        var lastTrendChange = _stateProvider.CurrentInventory.LastTrendChange;

        // Get candlestick data - need enough for both EMAs and ADX
        var requiredCandles = Math.Max(_config.EmaSlowPeriod, 14) + 30; // Extra for ADX warmup
        var candles = await _marketDataProvider.GetCandlesticksAsync(marketId, "1h", requiredCandles, ct);

        if (candles.Count < _config.EmaSlowPeriod)
        {
            _logger.LogWarning("Insufficient candle data for trend analysis on market {MarketId}. Got {Count}, need {Required}",
                marketId, candles.Count, _config.EmaSlowPeriod);
        ...
        var ema20 = _indicatorService.CalculateEma(closePrices, _config.EmaFastPeriod);
        var ema50 = _indicatorService.CalculateEma(closePrices, _config.EmaSlowPeriod);
        ...
        var reason = BuildTrendReason(ema20, ema50, macd, adx, proposedState);
        ...
        {
            confirmationTime = HandleConfirmation(marketId, proposedState);
        }
        ...
        if (!confirmationRequired && proposedState != currentState)
        {
            ...
            // TryRemove returns true if removed, with the removed value in 'pending'
            var wasRemoved = _pendingConfirmations.TryRemove(marketId, out var pending);

            if (wasRemoved)
            {
                // Check if conditions are met for confirmation
            ...
            // If wasRemoved is false, nothing was in the dictionary - no action needed
        }
    ...
    public TrendState DetermineTrendState(decimal ema20, decimal ema50, MacdResult macd, decimal adx)
    {
        ...
        if (emaProximityPercent < _config.EmaNeutralProximityPercent)
        {
            return TrendState.Neutral;
        }

        // ADX check: if ADX is below neutral threshold, trend is weak
        if (adx < _config.AdxNeutralThreshold)
        {
            return TrendState.Neutral;
        }
        ...
        var strongTrend = adx > _config.AdxStrongTrendThreshold;
    ...
    private DateTimeOffset HandleConfirmation(int marketId, TrendState proposedState)
    {
        var confirmationDelay = TimeSpan.FromMinutes(_config.ConfirmationDelayMinutes);
    ...
    private void RecordTrendFlip(int marketId, TrendState from, TrendState to)
    {
        ...
        if (recentFlips >= 2)
        {
            var cooldownExpiry = now.AddMinutes(_config.TrendFlipCooldownMinutes);
    ...
    private string BuildTrendReason(
        decimal ema20,
        decimal ema50,
        MacdResult macd,
        decimal adx,
        TrendState state)
    {
        ...
        var adxStrength = adx > _config.AdxStrongTrendThreshold ? "strong" : adx < _config.AdxNeutralThreshold ? "weak" : "moderate";
