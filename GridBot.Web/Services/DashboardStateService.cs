...
public sealed class DashboardStateService : IDashboardStateService
{
    ...
    public async Task AddAlertAsync(AlertItem alert, bool sendWebhook = true, CancellationToken ct = default)
    {
        ...
        if (sendWebhook)
        {
            ...
            var webhookService = scope.ServiceProvider.GetRequiredService<IWebhookNotificationService>();
            await webhookService.SendAlertAsync(alert, ct);
        }
    ...
    {
        await using var scope = _scopeFactory.CreateAsyncScope();
        var tradingApiClient = scope.ServiceProvider.GetRequiredService<TradingApiClient>();
    ...
    private async Task UpdateStateAsync(DashboardState newState, CancellationToken ct)
    {
        // Check for state changes that should trigger alerts
        await CheckForStateChangeAlertsAsync(newState, ct);
    ...
    private async Task CheckForStateChangeAlertsAsync(DashboardState newState, CancellationToken ct)
    {
            ...
            _previousTradingState != newState.TradingState)
        {
            ...
            var alert = new AlertItem
            {
                ...
                Severity = severity,
                EventType = WebhookEventTypes.StateChange,
            ...
            await AddAlertAsync(alert, sendWebhook: true, ct);
        }
            ...
            newState.RecoveryPhase != "None")
        {
            var alert = AlertItem.Info(
                WebhookEventTypes.RecoveryPhase,
            ...
            _currentState.ConsecutiveTimeouts < 3)
        {
            var alert = AlertItem.Warning(
                WebhookEventTypes.TimeoutWarning,
